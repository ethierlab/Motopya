...
int tmp_pos;
int trial_pos;
std::vector<std::vector<int>> tmp_value_buffer(lenBuffer, std::vector<int>(2, 0));    // [time value], first row is oldest data
std::vector<std::vector<int>> trial_value_buffer(lenBuffer, std::vector<int>(2, 0));  // [time value]
...
void reinitializeToZero(std::vector<std::vector<int>>& vec) {
  send_message("reinitializing");
    for (auto& element : vec) {
        // std::fill(element.begin(), element.end(), 0);
        element[0] = 0;
        element[1] = 0;
    }
  send_message("after reinitializing");
}
...
void recordCurrentValue() {
    ...
    // trial_value_buffer.push_back(values);
        ...
        // values.push_back(moduleValue_now);
    if (trial_pos >= lenBuffer) {
      sendTrialData2Python(false);
      reinitializeToZero(trial_value_buffer);
      trial_pos = 0;
      // trial_value_buffer.erase(trial_value_buffer.begin());
    }

    trial_value_buffer[trial_pos][0] = trial_time;
    if (trial_value_buffer[trial_pos][0] > 10000) {
      send_message("overflow?" + String(trial_value_buffer[trial_pos][0] > 10000));
    }
    trial_value_buffer[trial_pos][1] = moduleValue_now;
    trial_pos ++;
...
void stateMachine() {
  ...
  if (tmp_pos >= lenBuffer) {
    // tmp_value_buffer.erase(tmp_value_buffer.begin());
    // send_message("resetting tmp buffer");
    tmp_pos = 0;
  }

  tmp_value_buffer[tmp_pos][0] = session_t;
  tmp_value_buffer[tmp_pos][1] = moduleValue_now;
  tmp_pos ++;
  ...
  switch (CURRENT_STATE) {
    ...
    case STATE_TRIAL_INIT:
      ...
      if (tmp_value_buffer.size() > 0) {
        ...
        //   }
        // );
        int pos_dif = tmp_value_buffer.size() - tmp_pos;
        for (int i = 0; i < pos_dif; i++){
          trial_value_buffer[i][0] = tmp_value_buffer[i + tmp_pos][0] - trial_start_time;
          if (trial_value_buffer[i][0] > 10000) {
              send_message("overflow? -- Value: " + String(trial_value_buffer[i][0]) + " i : " + String(i) + " tmp_pos where it comes from" + String(tmp_pos));
          }
          trial_value_buffer[i][1] = tmp_value_buffer[i + tmp_pos][1];
        }
        for (int i = 0; i < tmp_pos; i++){
          trial_value_buffer[i + pos_dif][0] = tmp_value_buffer[i][0] - trial_start_time;
          if (trial_value_buffer[i][0] > 10000) {
              send_message("overflow? -- Value: " + String(trial_value_buffer[i][0]) + " i + pos_dif: " + String(i + pos_dif) + " i where it comes from" + String(i));
          }
          trial_value_buffer[i + pos_dif][1] = tmp_value_buffer[i][1];
        }
        sendTrialData2Python(false);
        reinitializeToZero(trial_value_buffer);
      }
...
unsigned long prevtimeee = millis();
void experimentOn() {
  ...
  while (serialCommand.charAt(0) != 'w' && serialCommand.charAt(0) != 'e') {
    unsigned long timeee = micros();
    // unsigned long timeee = TCNT1;
    int dif = timeee - prevtimeee;
    
    if (dif > 1000000) {
      digitalWrite(10, digitalRead(10) ^ 1);  //Turns LED ON and OFF
      // prevtimeee = timeee;
      prevtimeee = timeee;
    }
...
void reInitialize() {
  ...
  reinitializeToZero(tmp_value_buffer);// [time value], first row is oldest data
  reinitializeToZero(trial_value_buffer);// [time value

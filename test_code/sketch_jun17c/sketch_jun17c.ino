...
void recordCurrentValue() {
    ...
    if (trial_pos >= lenBuffer) {
      sendTrialData2Python(false);
      trial_value_buffer.clear();  // Clear existing content
      trial_value_buffer.resize(lenBuffer, std::vector<int>(2, 0));
      trial_pos = 0;
      send_message("trial_time");
      send_message(String(trial_time));
      // trial_value_buffer.erase(trial_value_buffer.begin());
...
void reInitialize() {
  ...
  tmp_value_buffer.resize(lenBuffer, std::vector<int>(2, 0));
